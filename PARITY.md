...
- [x] chart — their Recharts build-a-chart tutorial ported as five progressive Chart.js-option examples (Your First Chart → Add a Grid → Add an Axis → Add Tooltip → Add Legend, on their two-series desktop/mobile dataset) with the Recharts→Chart.js mapping table added to docs/06-CHARTS.md (ChartConfig ≈ dataset labels + --pk-chart tokens; theming already automatic via the theme observer). ChartContainer/accessibilityLayer/JSX tooltip content are Recharts-only by design
...
- [x] message-scroller — behavior verification PASSED: the controller already implements every documented concept (live-edge follow w/ reader-intent release, [data-scroll-anchor] turns settling w/ previousItemPeek, defaultPosition end/start/last-anchor, preserveOnPrepend history, jump button w/ direction, rAF smooth-scroll honoring reduced-motion). Added the floating jump-button CSS (data-active fade) + four interactive demos w/ inline simulated streams (Following the Live Edge, Anchoring Turns, Load History, Opening Position). React state demos (GroupChat/Commands) fold into these
...
- [x] sidebar — COMPLETE: collapsible: :icon rail mode (3rem strip, labels hide, icon-square buttons w/ pure-CSS data-tooltip hover bubbles; mobile falls back to the drawer), pk_sidebar_state cookie persistence (default_collapsed: renders server-side, no flash), SidebarRail edge grab-strip, ⌘B/Ctrl+B; offcanvas + static parts were PR #19. Icon-rail example added
