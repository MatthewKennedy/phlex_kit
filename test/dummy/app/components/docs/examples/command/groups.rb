            ...
            render PhlexKit::Command.new(data: { controller: "phlex-kit--command" }, style: "border: 1px solid var(--pk-border);") do
              ...
              render PhlexKit::CommandList.new do
                render PhlexKit::CommandGroup.new(title: "Suggestions") do
                  render PhlexKit::CommandItem.new(value: "calendar", href: "#") do
                    render PhlexKit::Icon.new(:calendar, size: nil)
                    span { "Calendar" }
                  end
                  render PhlexKit::CommandItem.new(value: "search emoji", href: "#") do
                    render PhlexKit::Icon.new(:search, size: nil)
                    span { "Search Emoji" }
                  end
                  render PhlexKit::CommandItem.new(value: "calculator", href: "#") do
                    render PhlexKit::Icon.new(:plus, size: nil)
                    span { "Calculator" }
                  end
                end
                render PhlexKit::CommandSeparator.new
